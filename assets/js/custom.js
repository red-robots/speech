  ...
  if (homePopupMessage.length) {
    ...
    if (message) {
      ...
      if (buttonOption.length) {
        ...
        button_obj = {
          ok: {
            ...
            action: function action(heyThereButton) {
              if (targetAtt) {
      ...
      $.confirm((_$$confirm = {
        ...
        theme: 'modern',
        closeIcon: true
      }, _defineProperty(_$$confirm, "draggable", false), _defineProperty(_$$confirm, "animation", 'zoom'), _defineProperty(_$$confirm, "buttons", button_obj), _defineProperty(_$$confirm, "onContentReady", function onContentReady() {}), _$$confirm));
    }
  }
  /* Select Style */
    ...
    if ($(".flightInfo ul li").length > 0) {
      ...
      $(".flightInfo ul li").each(function () {
        if ($(this).find(".listtext").length == 0) {
          ...
          if ($(this).find(".listtext").height() > maxHeight) {
            maxHeight = $(this).find(".listtext").height();
          }
        }
      });
      ...
      $.ajax({
        ...
        success: function success(res) {
          if ($(res).find('div.innerPosts .boxinfo').length) {
            ...
            $("#loaderdiv").removeClass("show"); //window.history.pushState("","",nextPageLink);
